...
import type { CacheTag } from "@repo/datocms";
import { deleteQueries, queriesReferencingCacheTags } from "@repo/datocms";
...
export async function POST(request: Request) {
  if (
    request.headers.get("Revalidate-Token") !==
    process.env.CACHE_INVALIDATION_SECRET_TOKEN
  ) {
    return NextResponse.json(
      {
        error:
          "You need to provide a secret token in the `Revalidate-Token` header for this endpoint.",
      },
      { status: 401 },
    );
  }
